...
const loopinteraion = (zx,zy,cx,cy,space)=> {
	...
	for(let i=0 ; i<maxIte; i++){
	  zSq  = square({ x:xval,y:yval })
		...
		if((zNxt.x**2 + zNxt.y**2 <= 4) && (i==maxIte-1))
		 	zMouseController ? finiteZ.push({x:zx,y:zy,v:value}) : finiteC.push({x:cx,y:cy,v:value})
  ...
  zMouseController ?  allZ.push({x:zx,y:zy,v:value}): allC.push({x:cx,y:cy,v:value})
}
...
const graph = space =>{
	...
	for(let i = 0; i<=height/2;i+=space/divider){
		text(i/space,i,0)
	}
