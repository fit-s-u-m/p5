...
function setup(){ 
	...
	const sinCosVButton      = makeButton("sin cos ","sinCosVButton",()=>{isSinCosVisualied=!isSinCosVisualied})
	const tanSecVButton      = makeButton("tan sec","tanSecVButton",()=>{isTanSecVisualied=!isTanSecVisualied})
	const cotCscVButton      = makeButton("cot csc","cotCscVButton",()=>{isCotCscVisualied=!isCotCscVisualied})
	...
	rad = createSlider(width/6,width/4).parent("button_container").class("buttons").id("rad")
...
function draw(){
  ...
  const ballRadius = width/80
    ...
    styleWhenButtonOff("#makeCircleButton")
		...
		nextX.length  =0 
		nextY.length  =0 
	// sinV(radius,ballRadius)
	// tanget(radius,ballRadius)
	}
	...
	circlingBallButtonIsOn	 ? showCirclingBall(x,y,xPoints,yPoints,ballRadius)          :styleWhenButtonOff("#circlingBallButton")
	...
	cosGraphButtonIsOn       ? showCosGraph(x,xPoints)						       :styleWhenButtonOff("#cosGraphButton")
	isSinCosVisualied        ? sinV(radius,ballRadius)						       :styleWhenButtonOff("#sinCosVButton")
	isTanSecVisualied        ? tanV(radius,ballRadius)						       :styleWhenButtonOff("#tanSecVButton")
	isCotCscVisualied        ? CotV(radius,ballRadius)						       :styleWhenButtonOff("#cotCscVButton")
...
const makeCircle = (x, y, radius) =>{
	...
	push()
		translate(0,0)
		box(radius*2,radius/30)
		box(radius/30,2*radius,radius/30)
	pop()
...
const sinV = (radius,ballRadius)=>{
	circleButtonIsOn=true
	styleWhenButtonOn("#sinCosVButton")
